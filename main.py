...
# Function to fetch on-chain DEX pool data
def get_dex_pools(query, network, include, page=1):
    headers = {"x-cg-pro-api-key": api_key}
    try:
        response = requests.get(
            f"https://pro-api.coingecko.com/api/v3/onchain/search/pools?query={query}&network={network}&include={include}&page={page}",
            headers=headers,
        )
        response.raise_for_status()
        data = response.json()
...
# command to get the on-chain DEX pool data
async def search_pools(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data and "data" in data:
        ...
        for i in range(0, len(message), 4096):
            await update.message.reply_text(
                message[i : i + 4096], parse_mode="Markdown"
            )
    else:
        await update.message.reply_text(
            f"Sorry, I could not fetch the data for the query {query} on the network {network}. Please check the query and network and try again."
        )
